...
const directmail = express.Router()

const BASE_URL = path.join(
  __dirname.split('\\').slice(0, -2).join('\\'),
  '/public/db/webdb/'
)
const DB_URL = BASE_URL + 'directmail.json'

directmail.get('/list', async(req, res, next) => {
  try {
...
directmail.patch('/update/:id', async(req, res, next) => {
  try {
...
directmail.delete('/delete/:id', async(req, res, next) => {
  try {
    const id = req.params.id
    let list = await readFile(DB_URL, 'utf-8')
    list = JSON.parse(list)
...
directmail.post('/add/', async(req, res, next) => {
  try {
    const id = req.params.id
